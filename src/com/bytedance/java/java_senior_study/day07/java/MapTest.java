 ...
 * @author yuhang.sun
 * @version 1.0
 * @date 2021/1/21 - 23:16
 * @Description
 * 一、Map的实现类结构：
 * |----Map：双列数据，存储key-value对的数据  ---类似于高中的函数：y=f(x)
 *    |----HashMap：作为Map的主要实现类：线程不安全的，效率高；存储null的key和value的
 *       |----LinkedHashMap：保证在遍历map元素时，可以按照添加的顺序实现遍历。
 *                       原因：在原有的HashMap底层结构基础上，添加了一对指针，指向前一个和后一个元素。
 *                       对于频繁的遍历操作，此类执行效率高于HashMap
 *    |----TreeMap:保证按照添加的key-value进行排序，实现排序遍历。此时考虑key的自然排序或定制排序。
 *                 底层使用红黑树
 *    |----Hashtable：作为Map的古老实现类：线程安全的，效率低；不能存储null的key和value的
 ...
 *  3.ConCurrentHashMap与Hashtable的异同？(暂时不讲)
 *
 *  二、Map结构的理解：
 ...
 *         如果key1的hash值与已经存在的某一个数据(key2-value2)的hash值相同，继续比较，调用key1所在类的equals(key2)方法
 *            如果equals()返回false：此时key1-value1添加成功。 --- 情况3
 *            如果equals()返回true：使用value1替换value2。
 *
 *   补充：关于情况2和情况3：此时key1-value1和原来的数据以链表的方式存储。
 *
 *   在不断地添加过程中，会涉及到扩容问题，当超出临界值(且要存放的位置非空)时，默认的扩容方式：扩容为原来容量的2倍，并将原有的数据复制过来。
 *
