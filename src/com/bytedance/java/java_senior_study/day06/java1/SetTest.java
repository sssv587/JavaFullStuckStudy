 ...
 *   要求：重写的hashCode()和equals()尽可能保证一致性：相等的对象必须具有相等的散列码
...
public class SetTest {
    ...
    一、Set：存储无序的、不可重复的数据
    以HashSet为例说明：
    1.无序性：不等于随机性。存储的数据在底层数组中并非按照数组索引的顺序添加，而是根据数据组的哈希值决定的。
    ...
    //LinkedHashSet的使用
    //LinkedHashSet作为HashSet的子类，在添加数据的同时，每个数据还维护了一对两个引用，记录此数据前一个
    //数据和后一个数据。
